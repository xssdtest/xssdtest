...
rootdir=$(readlink -f $(dirname $0))"/Samanea_Platform/spdk"
...
function configure_linux_pci {
	...
	grep "PCI_DEVICE_ID_INTEL_IOAT" $curdir/pci_ids.h \
	| awk -F"x" '{print $2}' > $TMP
	...
	grep "PCI_DEVICE_ID_VIRTIO" $curdir/pci_ids.h \
	| awk -F"x" '{print $2}' > $TMP
	...
	grep "PCI_DEVICE_ID_INTEL_VMD" $curdir/pci_ids.h \
	| awk -F"x" '{print $2}' > $TMP
...
function status_linux {
	...
	#collect all the device_id info of ioat devices.
	TMP=$(grep "PCI_DEVICE_ID_INTEL_IOAT" $curdir/pci_ids.h \
	| awk -F"x" '{print $2}')
	...
	#collect all the device_id info of virtio devices.
	TMP=$(grep "PCI_DEVICE_ID_VIRTIO" $curdir/pci_ids.h \
	| awk -F"x" '{print $2}')
	echo -e "BDF\t\tVendor\tDevice\tNUMA\tDriver\t\tDevice name"
	for dev_id in $TMP; do
		for bdf in $(iter_all_pci_dev_id 1af4 $dev_id); do
	...
	#collect all the device_id info of vmd devices.
	TMP=$(grep "PCI_DEVICE_ID_INTEL_VMD" $curdir/pci_ids.h \
	| awk -F"x" '{print $2}')
...
if [ -z "$pciPort" ]; then
	pciPort=$(iter_all_pci_class_code 01 08 02)
else
  pciPort=$pciPort
fi
