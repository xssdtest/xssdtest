...
void xt_sha256_init(struct xt_sha256_ctx *sctx)
{
	sctx->state[0] = H0;
	sctx->state[1] = H1;
	sctx->state[2] = H2;
...
void xt_sha256_update(struct xt_sha256_ctx *sctx, const uint8_t *data,
		       unsigned int len)
{
...
void xt_sha256_final(struct xt_sha256_ctx *sctx)
{
	uint64_t bits;
	...
	xt_sha256_update(sctx, padding, pad_len);

	/* Append length (before padding) */
	xt_sha256_update(sctx, (const uint8_t *)&bits, sizeof(bits));
